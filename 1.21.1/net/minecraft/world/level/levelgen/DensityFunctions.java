    ...
    ) implements DensityFunctions.TwoArgumentSimpleFunction
    {
        ...
        public double compute(DensityFunction.FunctionContext p_208410_)
        {
            ...
            return switch (this.type)
            {
                ...
                case MUL -> d0 == 0.0 ? 0.0 :
                        d0 * this.argument2.compute(p_208410_);

                case MIN -> d0 < this.argument2.minValue() ? d0 :
                        Math.min(d0, this.argument2.compute(p_208410_));

                case MAX -> d0 > this.argument2.maxValue() ? d0 :
                        Math.max(d0, this.argument2.compute(p_208410_));
            };
        }
    ...
    implements DensityFunctions.PureTransformer
    {
        ...
        private static double transform(DensityFunctions.Mapped.Type p_208669_, double p_208670_)
        {
            ...
            return switch (p_208669_)
            {
                ...
                case HALF_NEGATIVE -> p_208670_ > 0.0 ? p_208670_ :
                        p_208670_ * 0.5;

                case QUARTER_NEGATIVE -> p_208670_ > 0.0 ? p_208670_ :
                        p_208670_ * 0.25;
    ...
    interface TwoArgumentSimpleFunction extends DensityFunction
    {
        ...
        )
        {
            ...
            double d5 = switch (p_209074_)
            {
                ...
                case MUL -> d0 > 0.0 && d1 > 0.0 ? d0 * d1 :
                        (d2 < 0.0 && d3 < 0.0 ? d2 * d3 : Math.min(d0 * d3, d2 * d1));
            ...
            double d4 = switch (p_209074_)
            {
                ...
                case MUL -> d0 > 0.0 && d1 > 0.0 ? d2 * d3 :
                        (d2 < 0.0 && d3 < 0.0 ? d0 * d1 : Math.max(d0 * d1, d2 * d3));
