...
public class Font implements IForgeFont
{
        ...
        for (int j = -1; j <= 1; j++)
        {
            for (int k = -1; k <= 1; k++)
            {
                if (j != 0 || k != 0)
                {
                    ...
                    int l = j;
                    int i1 = k;
                    p_168646_.accept((indexIn, styleIn, charIn) ->
                    {
    ...
    )
    {
        ...
        Matrix4f matrix4f = this.matrixShadow.set(p_272662_);
    ...
    )
    {
        ...
        Matrix4f matrix4f = this.matrixShadow.set(p_273265_);
    ...
    class StringRenderOutput implements FormattedCharSink
    {
        ...
        )
        {
            ...
            this.pose = MathUtils.isIdentity(p_254510_) ? BakedGlyph.MATRIX_IDENTITY : p_254510_;
            this.mode = p_181371_;
        ...
        public boolean accept(int p_92967_, Style p_92968_, int p_92969_)
        {
            ...
            if (textcolor != null)
            {
                ...
                f1 = (float)(i >> 16 & 0xFF) / 255.0F * this.dimFactor;
                f2 = (float)(i >> 8 & 0xFF) / 255.0F * this.dimFactor;
                f3 = (float)(i & 0xFF) / 255.0F * this.dimFactor;
            }
            else
            {
                f1 = this.r;
            ...
            if (p_92968_.isStrikethrough())
            {
                this.addEffect(
                    new BakedGlyph.Effect(
                        this.x + f7 - 1.0F, this.y + f7 + 4.5F, this.x + f7 + f6, this.y + f7 + 4.5F - 1.0F, 0.01F, f1, f2, f3, f
                    )
                );
            }
            ...
            if (p_92968_.isUnderlined())
            {
                this.addEffect(
                    new BakedGlyph.Effect(
                        this.x + f7 - 1.0F, this.y + f7 + 9.0F, this.x + f7 + f6, this.y + f7 + 9.0F - 1.0F, 0.01F, f1, f2, f3, f
                    )
                );
            }
