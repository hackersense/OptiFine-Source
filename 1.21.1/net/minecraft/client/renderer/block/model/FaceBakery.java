...
public class FaceBakery
{
    ...
    )
    {
        ...
        boolean flag = Reflector.ForgeHooksClient_fillNormal.exists() ? false : p_111608_;
        int[] aint = this.makeVertices(blockfaceuv, p_111604_, p_111605_, this.setupShape(p_253895_, p_253976_), p_111606_.getRotation(), p_111607_, flag);
        ...
        if (Reflector.ForgeHooksClient_fillNormal.exists() && Reflector.ForgeBlockElementFace_data.exists())
        {
            ForgeFaceData forgefacedata = (ForgeFaceData)Reflector.call(p_111603_, Reflector.ForgeBlockElementFace_data);
            ReflectorForge.fillNormal(aint, direction, forgefacedata);
            BakedQuad bakedquad = new BakedQuad(aint, p_111603_.tintIndex(), direction, p_111604_, p_111608_, forgefacedata.ambientOcclusion());
    ...
    )
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            this.bakeVertex(aint, j, p_111576_, p_111574_, p_111577_, p_111575_, p_111578_, p_111579_, p_111580_);
        }
    ...
    private void fillVertex(int[] p_111615_, int p_111616_, Vector3f p_254291_, TextureAtlasSprite p_111618_, BlockFaceUV p_111619_)
    {
        ...
        int j = p_111616_ * i;
        p_111615_[j] = Float.floatToRawIntBits(p_254291_.x());
        p_111615_[j + 1] = Float.floatToRawIntBits(p_254291_.y());
        p_111615_[j + 2] = Float.floatToRawIntBits(p_254291_.z());
        p_111615_[j + 3] = -1;
        p_111615_[j + 4] = Float.floatToRawIntBits(p_111618_.getU(p_111619_.getU(p_111616_) / 16.0F));
        p_111615_[j + 4 + 1] = Float.floatToRawIntBits(p_111618_.getV(p_111619_.getV(p_111616_) / 16.0F));
    }
